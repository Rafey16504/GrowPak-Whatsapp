...
df["roman_urdu_auto"] = df["roman_urdu_auto"].apply(clean)
...
df["WER_auto"] = df.apply(
    lambda r: wer(str(df_raw.loc[r.name, "ground_truth"]), 
                  str(df_raw.loc[r.name, "roman_urdu_auto"])), axis=1)
